...
var SimpleBackend = require('./simplebackend');
...
var Crawler = function(opts) {
	...
	this.fetch = function(link) {
		...
		if (link && isUrl.exec(link)) {
			...
			if (this._contains(link, this.treeRoutes)) {
	...
	this.ignoreRoutes = this._routeToExp(opts.routes.ignore.map(function(route) {
		var o = {};
		o[route] = function() {
	...
	//Make the backend
	this.q = opts.queue || new SimpleBackend.Backend();
